    ...
    return {
        ...
        updateMediaMessage: async (message) => {
           ...
           sendMessage: async (jid, content, options = {}) => {
            ...
            const { filter = false, quoted, ephemeralExpiration, disappearingMessagesInChat } = options;
            const getParticipantAttr = () => filter ? { participant: { jid } } : {};
            const messageType = luki.detectType(content);
            
            // Handle disappearing messages for both DMs and groups
            if (disappearingMessagesInChat || ephemeralExpiration) {
                const expiration = ephemeralExpiration || (typeof disappearingMessagesInChat === 'number' ? disappearingMessagesInChat : Defaults_1.WA_DEFAULT_EPHEMERAL);
                if (WABinary_1.isJidGroup(jid)) {
                    await groupToggleEphemeral(jid, expiration);
                }
                // Set expiration in contextInfo for the message
                if (!content.contextInfo) content.contextInfo = {};
                content.contextInfo.expiration = expiration;
            }
